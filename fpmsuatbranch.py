...
def handle_lark_fpms_uat_branch_message(
    ...
    *,
    ...
    Follow-up replies (digits / yes / no) use an existing session and do not require mention.
    ...
    if sess is not None:
        ...
        if st == "pick":
            ...
            if sess["pick_index"] >= len(sess["service_tokens"]):
                ...
                send(chat_id, _fpms_format_summary_message(sess["data"], sess["resolved_ids"]))
                return True
