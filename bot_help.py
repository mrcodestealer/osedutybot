    ...
    sections: list[HelpSection] = [
        ...
        (
            ...
            [
                ...
                ("deleteoffset", "Delete your offset; or “cancel my offset”", "删除调休"),
        ...
        [
            (
                ...
                [
                    ...
                    ("/sports <notice>", "Like /egs but emails CS (Team) + Toby (Cc CP OM Duty + Allan Guo)", "同 /egs，发给 CS (Team) + Toby（抄送 CP OM Duty + Allan Guo）"),
                    ("/sportstest <notice>", "Like /sports but test send → junchen@", "同 /sports，测试发到 junchen@"),
                    ("/sportsreply <body>", "Paste reply body → pick a sent /sports email → reply in its thread", "粘贴回复正文 → 选择已发 /sports 邮件 → 原邮件内回复"),
                    ("/sportsreplytest <body>", "Like /sportsreply but reply → junchen@ (test); /sportstestreply also works", "同 /sportsreply，回复发到 junchen@（测试）；也可用 /sportstestreply"),
                    ...
                    ("/showenv", "Admin only: the bot's .env file (sent to your private chat)", "仅管理员：查看机器人 .env（私聊发送）"),
                    ("/addenv KEY=value", "Admin only: append a new line to .env (restart to apply)", "仅管理员：在 .env 末尾新增一行（重启后生效）"),
                    ("/editenv [text]", "Admin only: edit .env values in a card, Save overwrites (backup kept)", "仅管理员：卡片中编辑 .env 值，保存即覆盖（保留备份）"),
                    ...
                    ("/vacheck [force] [provider]", "Read a provider's group now (VA by default; Telegram or Teams); an upcoming maintenance window fills its row in the Base + cards Laboratory", "立即读取供应商群（默认 VA；Telegram 或 Teams）；若有即将维护的时间段，则填入维护表并发卡片"),
