...
from holiday import get_today_date, format_holidays
...
def lark_webhook():
    ...
    if len(clean_text) >= 3 and clean_text[:3].lower() == '/s ':
    ...
    elif clean_text.lower() in ['/fpms', '/bi', '/sre', '/db', '/dba', '/fe', '/cpms', '/pms']:
        dept_map = {
            '/fpms': 'fpms',
            '/bi': 'bi',
            '/sre': 'sre',
        ...
        reply = "I only respond to `/help`, `/s <name>`, `/date`, `/holiday`, `/duty`, or department commands like `/fpms`, `/bi`, `/sre`, `/db`, `/dba`, `/fe`, `/cpms`, `/pms`."
