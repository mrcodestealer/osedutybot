...
SPREADSHEET_TOKEN = os.getenv("MDR_SPREADSHEET_TOKEN")
SHEET_ID = os.getenv("MDR_SHEET_ID")
...
def get_mdr_info(query_str):
    # Extract numbers from query (mdr prefix optional)
    numbers = re.findall(r'(?i)mdr?(\d+)', query_str)
    if not numbers:
        numbers = re.findall(r'\b(\d{4,5})\b', query_str)   # accepts 4‑5 digit numbers
    if not numbers:
        return "❌ No valid Asset IDs found. Use e.g., `/mdr 1234` or `/mdr mdr1234 mdr5678`"
    ...
    for asset_id in numbers_unique:
        ...
        if row:
            output_lines.append(f"📌 Machine : MDR{asset_id}")
            output_lines.append("")
