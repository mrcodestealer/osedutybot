...
**Train / test**
    python ai.py train [--epochs 8] [--output command_intent_pt]
    python ai.py test "who is on fpms duty today"
    python ai.py eval
    python ai.py patterns   # print training pattern counts
"""
...
_SAMPLE_MACHINE_IDS = ("1422", "8900", "2133", "NCH1422", "NWR2140")
...
def _cli_test(phrase: str, model_path: Path) -> None:
    if not (model_path / "config.json").is_file():
        print(f"Model not found at {model_path}. Run: python ai.py train")
        sys.exit(1)
    clf = CommandClassifier(model_path)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Duty Bot AI command router")
