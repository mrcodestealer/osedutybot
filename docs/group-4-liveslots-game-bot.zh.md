...
| `/isp <ip> [ip ...]` | IP 归属：ISP / 国家 / ASN；可配对粘贴的玩家 ID |
...
iplocation.net），回复 **IP Details** 卡片。多个地址按**网络分组**展示，运营商 / ASN /
国家与对应地址一一对应。每行标签给出主要来源与其余认同的来源数（`IPinfo +2`）。内网、
回环、保留地址不发起网络请求，直接说明原因。

从日志粘贴？地址旁边的**玩家 ID** 会自动配对，显示为 `138.84.76.76 👤1081561491`。
每组放一行或用空行分隔即可，**顺序不限**（先 ID 后 IP 也能正确配对）：
