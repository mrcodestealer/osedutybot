...
TBR Asset Lookup
Usage:
    /tbr <machine_id(s)>   e.g. /tbr 2099
    /tbr2099               (no space also works)
    /tbr tbr2099 tbr2100
    /tbr 2099,2100

TBR rows live on a tab of the same spreadsheet as TBP
(https://casinoplus.sg.larksuite.com/wiki/O325woCdtilgJwksAvLlYkNDgKg?sheet=SUBY9G),
so TBR_SPREADSHEET_TOKEN falls back to TBP_SPREADSHEET_TOKEN and
TBR_SHEET_ID defaults to SUBY9G.
"""
...
SPREADSHEET_TOKEN = os.getenv("TBR_SPREADSHEET_TOKEN") or os.getenv("TBP_SPREADSHEET_TOKEN")
SHEET_ID = os.getenv("TBR_SHEET_ID") or "SUBY9G"
...
def get_tbr_info(query_str):
    # Extract numbers from query (tbr prefix optional)
    numbers = re.findall(r'(?i)tbr\s*-?\s*(\d+)', query_str)
    if not numbers:
        numbers = re.findall(r'\b(\d{4})\b', query_str)   # exactly 4 digits
    if not numbers:
        return "❌ No valid 4‑digit Machine IDs found. Use e.g., `/tbr 2099` or `/tbr tbr2099 tbr2100`"
    ...
    for machine_id in numbers_unique:
        ...
        if row:
            output_lines.append(f"📌 Machine : TBR{machine_id}")
            output_lines.append("")
